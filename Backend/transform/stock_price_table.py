...
REPO_ROOT = HERE.parents[2]   # build_prices_year_partitioned.py -> download -> Backend -> project root
DATASETS_DIR = REPO_ROOT / "Datasets"

PRICES_DS_ROOT = DATASETS_DIR / "yahoo_prices_ds"
MANIFEST_PATH = PRICES_DS_ROOT / "_manifest.csv"
RUN_META_PATH = PRICES_DS_ROOT / "_run_meta.csv"

TICKER_SOURCE_PATH = DATASETS_DIR / "data_with_tickers.parquet"
TICKER_COL = "ticker"
...
def load_tickers_from_parquet(parquet_path: Path, ticker_col: str, strict: bool = True) -> list[str]:
    if not parquet_path.exists():
        raise FileNotFoundError(f"Ticker source parquet not found: {parquet_path}")

    tbl = pq.read_table(parquet_path, columns=[ticker_col])
    s = tbl.column(ticker_col).to_pandas()
    ...
    if strict:
        ...
        tickers = tickers[
            (tickers != "")
            & (~tickers.str.contains(r"\^|/|=", regex=True))
        ]
...
def get_status(manifest: pd.DataFrame, year: int, ticker: str) -> Optional[str]:
    if manifest.empty:
        return None
...
def year_is_resolved(
    ...
    """
    A year is considered resolved when every ticker is either:
    - ok
    - nodata
    - retry but already at/above max attempts
    """
    if not tickers:
        return True
...
def infer_resume_start_year(
    ...
    """
    Return the first year that is not fully resolved.
    If all years are resolved, return end_year + 1.
    """
...
def yahoo_prices_chunk(
    ...
    """
    Download one chunk of tickers from Yahoo.
    Retries are for chunk-level failures only.
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Critical fix:
            # remove rows where Yahoo gave a date index but no real price/volume data
...
def compact_year_partition(root: Path, year: int, compression: str = "zstd") -> None:
    """
    Merge many chunk files in year=YYYY into one compact file.
    Safe to run after that year is mostly complete.
    """
    part_dir = _partition_dir(root, year)
    ...
    df = (
        df.drop_duplicates(subset=["ticker", "date"], keep="last")
          .sort_values(["ticker", "date"])
          .reset_index(drop=True)
    )

    compact_path = part_dir / f"part-{year}-compact.parquet"
    table = pa.Table.from_pandas(df, preserve_index=False)
    pq.write_table(table, str(compact_path), compression=compression)
...
def build_prices_dataset(
    ...
    for year in range(cfg.start_year, cfg.end_year + 1):
        ...
        if year_is_resolved(manifest, year, tickers, cfg.max_attempts_per_ticker_year):
            ...
            if cfg.compact_after_year:
                compact_year_partition(cfg.out_root, year)
            continue
    ...
    meta = pd.DataFrame([{
        ...
        "max_attempts_per_ticker_year": cfg.max_attempts_per_ticker_year,
        "compact_after_year": cfg.compact_after_year,
...
def main() -> None:
    tickers = load_tickers_from_parquet(
    ...
    cfg = BuildConfig(
        ...
        interval="1d",
        chunk_size=20,
        mode="fresh",              # change to "update" for resume runs
        limit_tickers=None,
        strict_ticker_filter=True,
